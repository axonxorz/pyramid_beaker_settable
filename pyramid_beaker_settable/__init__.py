...
def BeakerSessionFactoryConfig(**options):
    ...
    class PyramidBeakerSessionObject(SessionObject):
                ...
                if (exception is None or self._cookie_on_exception
                    ...
                    if headers['set_cookie'] and headers['cookie_out']:
                        response.headerlist.append(
                            ('Set-Cookie', str(headers['cookie_out'])))
        ...
        # Custom ID loader methods
        def set_by_id(self, newId):
            """Load a specific session id and update the cookie to use that from now on.
            Any values that need to be carried over to the new session must be extracted
            by the application before calling this."""
            self._session().id = newId
            self._session().clear()
            self._session().load()
            self._session()._set_cookie_values()
            self._session()._update_cookie_out(True)
